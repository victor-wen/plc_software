...
public class UiLayoutLoaderTests
{
    ...
    public void Full_layout_parses_all_module_types_and_actions()
    {
        ...
        Assert.Equal(UiActionKind.Navigate, menu.Buttons[1].Action!.Kind);
        Assert.Equal("position-loading", menu.Buttons[1].Action.Page);
        Assert.Equal(UiActionKind.Command, menu.Buttons[0].Action!.Kind);
        Assert.Equal(2, menu.Buttons[0].Action.Writes.Count);
        Assert.Equal(CommandTarget.AutoMode, menu.Buttons[0].Action.Writes[0].ResolveTarget());
        Assert.False(menu.Buttons[0].Action.Writes[0].Value);
        ...
        Assert.Equal("D201", group.Rows[0].Position!.Register);
        Assert.Equal(1000, group.Rows[0].Position.Max);
        Assert.Equal(20, group.Rows[0].Speed!.Max);
