...
/// <see cref="RegisterDecoder.DecodeProcess"/> and the params block with
/// <see cref="RegisterDecoder.DecodeParams"/>, and the three partial dictionaries are merged and
...
internal sealed class SnapshotCoordinator
{
    ...
    private IReadOnlyDictionary<string, object?> _params = new Dictionary<string, object?>();
    ...
    private void OnResult(PollingResult result)
    {
        ...
        else if (result.Group.Name == "Params")
        {
            _params = RegisterDecoder.DecodeParams(result.Registers);
        }
        else
        {
            return;
        ...
        var mergedProcess = new Dictionary<string, object?>(_process);
        foreach (var kv in _params)
        {
            mergedProcess[kv.Key] = kv.Value;
        }

        _merger.Publish(_fast, mergedProcess, DateTime.UtcNow);
    }
}
